...
export default function MyClassesPage() {
  ...
  const fetchClasses = useCallback(async () => {
    ...
    try {
      const res = await fetchSecure(`/api/trainer/${session.user.email}/classes`);
      if (res.ok) {
  ...
  const confirmDelete = async (e) => {
    ...
    try {
      ...
      const res = await fetchSecure(`/api/classes/${deletingClass._id}`, {
        method: "DELETE"
      });
  ...
  const handleViewAttendees = async (classId) => {
    ...
    try {
      const res = await fetchSecure(`/api/classes/${classId}/attendees`);
      if (res.ok) {
        const data = await res.json();
        setAttendees(data);
      }
  ...
  const handleUpdateClass = async (e) => {
    e.preventDefault();
    try {
      const res = await fetchSecure(`/api/classes/${editingClass._id}`, {
        method: "PUT",
